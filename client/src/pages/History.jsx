...
const History = () => {
  ...
  useEffect(() => {
    const myNotes = async () => {
      try {
        const res = await axios.get(`${serverUrl}/notes/getnotes`, {
  ...
  const openNotes = async (noteId) => {
    setLoading(true);
    try {
      const res = await axios.get(`${serverUrl}/notes/${noteId}`, {
        withCredentials: true,
      });
  ...
  `}
                    >
                      ...
                      <div className="flex flex-wrap gap-2 mt-2 text-xs">
                        ...
                        {t.examType && (
                          <span className="px-2 py-0.5 rounded-full bg-indigo-500/20 text-purple-300">
